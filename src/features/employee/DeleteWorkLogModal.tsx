...
export default function DeleteWorkLogModal({ entry, onClose, onDeleted }: DeleteWorkLogModalProps) {
  ...
  async function handleConfirm() {
    ...
    setError(null);
    try {
      await deleteMyWorkEntry(entry.id);
      onDeleted();
    } catch (err) {
  ...
  return (
    <div style={overlayStyle} onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div style={modalStyle} role="dialog" aria-modal="true" aria-labelledby="delete-log-title">
        <div style={headerStyle}>
          <h2 id="delete-log-title" style={titleStyle}>Delete Work Log</h2>
...
const modalStyle: CSSProperties = {
  ...
  boxShadow: "0 20px 60px rgba(0,0,0,0.2)", width: "100%", maxWidth: 420, overflow: "hidden",
};
const headerStyle: CSSProperties = {
  display: "flex", alignItems: "center", justifyContent: "space-between",
  padding: "var(--space-5) var(--space-6)", borderBottom: "1px solid var(--color-border)",
};
const titleStyle: CSSProperties = { fontSize: "var(--font-size-md)", fontWeight: 700, color: "var(--color-danger-text)" };
...
const bodyStyle: CSSProperties = { padding: "var(--space-5) var(--space-6)" };
const messageStyle: CSSProperties = { fontSize: "var(--font-size-sm)", color: "var(--color-text-muted)", marginBottom: "var(--space-4)" };
...
const previewLabelStyle: CSSProperties = { color: "var(--color-text-muted)", fontWeight: 500 };
